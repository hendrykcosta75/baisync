...
pub async fn update_assistant(
    ...
    // W1.2 thresholds — only overwrite when explicitly provided.
    let config_max_tool_rounds = req
        .config_max_tool_rounds
        .or(existing.config_max_tool_rounds);
    ...
    // W1.2 thresholds UPDATE. Writing NULL is a valid operation (keeps
    // behaviour fall-back to code defaults of 5 rounds / 30000 ms).
    db.query_unpaged(
