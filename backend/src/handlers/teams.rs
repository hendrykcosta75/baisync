...
pub async fn create(
    ...
    if role != "owner" && role != "admin" {
        return Err(AppError::Forbidden(
            "Apenas administradores podem criar equipes".into(),
        ));
    }
...
pub async fn update(
    ...
    if role != "owner" && role != "admin" {
        return Err(AppError::Forbidden(
            "Apenas administradores podem editar equipes".into(),
        ));
    }
...
pub async fn delete(
    ...
    if role != "owner" && role != "admin" {
        return Err(AppError::Forbidden(
            "Apenas administradores podem excluir equipes".into(),
        ));
    }
...
pub async fn list_members(
    ...
    let members_with_load: Vec<serde_json::Value> = members
        ...
        .map(|m| {
            let assigned = tasks
                .iter()
                .filter(|t| t.assignee_id == Some(m.user_id))
                .count();
            let done = tasks
                .iter()
                .filter(|t| t.assignee_id == Some(m.user_id) && t.status == "done")
                .count();
...
pub async fn add_member(
    ...
    if role != "owner" && role != "admin" {
        return Err(AppError::Forbidden(
            "Apenas administradores podem gerenciar membros".into(),
        ));
    }
    ...
    let data = serde_json::to_string(&json!({ "team_id": team_id, "user_id": body.user_id }))
        .unwrap_or_default();
    publish_to_workspace(
        &db,
        &team.workspace_id,
...
pub async fn remove_member(
    ...
    let data = serde_json::to_string(&json!({ "team_id": team_id, "user_id": user_id }))
        .unwrap_or_default();
    publish_to_workspace(
        &db,
        &team.workspace_id,
...
pub async fn create_task(
    ...
    let due_date = body
        .due_date
        .map(|d| scylla::frame::value::CqlTimestamp(d.timestamp_millis()));
    ...
    let task = team_service::create_task(
        ...
        due_date,
        &body.tags,
        body.okr_objective_id.as_ref(),
        body.okr_key_result_id.as_ref(),
    )
    .await?;
    ...
    let data =
        serde_json::to_string(&json!({ "task": &task, "team_id": team_id })).unwrap_or_default();
    publish_to_workspace(
        &db,
        &team.workspace_id,
        &auth_user.user_id,
...
pub async fn update_task(
    ...
    let data =
        serde_json::to_string(&json!({ "task": &updated, "team_id": team_id })).unwrap_or_default();
    publish_to_workspace(
        &db,
        &team.workspace_id,
        &auth_user.user_id,
...
pub async fn delete_task(
    ...
    if role != "owner" && role != "admin" {
        return Err(AppError::Forbidden(
            "Apenas administradores podem excluir tarefas".into(),
        ));
    }
    ...
    let data = serde_json::to_string(&json!({ "task_id": task_id, "team_id": team_id }))
        .unwrap_or_default();
    publish_to_workspace(
        &db,
        &team.workspace_id,
...
pub async fn create_comment(
    ...
    let comment =
        team_service::create_comment(&db, &task_id, &auth_user.user_id, &user_name, &body.content)
            .await?;
...
pub async fn download_task_attachment(
    ...
    let (file_name, mime_type, data) =
        team_service::get_task_attachment_data(&db, &task_id, &attachment_id).await?;
